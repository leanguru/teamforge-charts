...
for line in planning_folder_trace:
    ...
    while len(folder_trace) > 1:
        id = folder_trace[0]  
	if id not in [i for i in planning_folder_lookup.keys() if 'planningFolderTrace' in planning_folder_lookup[i].keys()]: 
            planning_folder_lookup[id]['planningFolderTrace'] = folder_trace
            planning_folder_lookup[id]['planningFolderDepth'] = len(folder_trace)
            planning_folder_lookup[id]['planningFolderPath'] = '>'.join(map(lambda x: planning_folder_lookup[x]['planningFolderTitle'],reversed(folder_trace[:-1])))
